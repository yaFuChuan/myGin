  ...
  const app = {
    ...
    async fetchAndRenderReport(cusnos, d1, d2) {
      ...
      data.forEach(item => {
        ...
        const formattedDT = `${rocYear}/${mm}/${dd} ${hh}:${mi}:${ss}`;
        header.innerHTML = `
          <h1>巨興化學儀器有限公司</h1>
          <h2>對帳單</h2>
          <p><strong>客戶代碼：${cusno}</strong> ${cname}</p>
          <p>期間：${d1} 至 ${d2}</p>
          <p>製表日期：${formattedDT}</p>
        `;
        ...
        footerNote.innerHTML = `<p>備註：貨款未全部兌現前，依動產法第三章規定，貨物所有權仍歸賣方所有，買方不得異議。</p>`;
        ...
        pageNumberDiv.innerText = '第 1 頁, 共 1 頁';
